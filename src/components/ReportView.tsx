...
import type { NeighborhoodReport } from "@/lib/report/build";
import { T, CONFIDENCE_LABEL_TR, FRESHNESS_LABEL_TR, ADMIN_LEVEL_TR } from "@/lib/i18n/tr";
import NeighborhoodMap from "@/components/NeighborhoodMap";

/**
 * Shared report renderer (§19.3). Used by both the neighborhood report
 * (`/n/[slug]`) and the arbitrary-point report (`/nokta`). `profileHref` lets
 * each route keep the lifestyle selector on its own URL.
 */
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      ...
      <section className="mt-8 grid gap-4 sm:grid-cols-3">
        <ListCard title={T.report.strengths} items={score.strengths} tone="good" empty={T.report.noStrengths} />
        <ListCard title={T.report.weaknesses} items={score.weaknesses} tone="warn" empty={T.report.noWeaknesses} />
        <ListCard
      ...
      <section className="mt-10">
            ...
            {businesses.map((b, i) => (
              <li key={i} className="flex items-center justify-between gap-4 px-4 py-2.5 text-sm">
                ...
                <span className="shrink-0 tabular-nums text-muted">
                  ≈ {b.distanceMeters} m ·{" "}
                  {b.walkEstimated ? T.report.walkMin(b.walkMinutes) : T.report.walkWithin(b.walkMinutes)}
                </span>
              </li>
            ))}
          </ul>
        ...
        <details className="mt-3 rounded-lg border border-line bg-surface-2 px-4 py-3 text-sm">
          <summary className="cursor-pointer font-medium text-ink">{T.report.walkExplainerTitle}</summary>
